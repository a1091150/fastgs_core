...
help:
	...
	@printf "  make swift-recorded-full-512-camera-references  Generate per-camera full-point 512x512 recorded references.\n"
...
swift-recorded-full-512-camera-references:
	/bin/zsh -lc 'for ((index=0; index<$(SWIFT_RECORDED_CAMERA_COUNT); index++)); do padded=$$(printf "%03d" $$index); conda run -n $(CONDA_ENV) python swift/FastGSSwiftTools/generate_recorded_reference.py --dataset-dir $(SWIFT_RECORDED_DATASET) --width 512 --height 512 --max-points 0 --eval-index $$index --out-dir $(SWIFT_RECORDED_FULL_512_REF_DIR)/camera_$$padded; done'
