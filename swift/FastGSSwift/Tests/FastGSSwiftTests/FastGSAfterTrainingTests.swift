...
final class FastGSAfterTrainingTests: XCTestCase {
    func testOpacityCapClampsDirectOpacityValues() throws {
        ...
        assertAfterTrainingClose(result.parameters.opacities.asArray(Float.self), [0.2, 0.82, 0.82])
    ...
    func testOpacityResetClearsOnlyOpacityOptimizerState() throws {
        ...
        assertAfterTrainingClose(result.parameters.opacities.asArray(Float.self), [0.2, 0.5, 0.5])
        assertAfterTrainingClose(updatedState.opacities.firstMoment.asArray(Float.self), [0, 0, 0])
        assertAfterTrainingClose(updatedState.opacities.secondMoment.asArray(Float.self), [0, 0, 0])
...
private func makeAfterTrainingParameters() -> FastGSTrainableParameters {
    FastGSTrainableParameters(
        ...
        opacities: MLXArray([Float(0.2), 0.9, 0.82], [3]),
...
private func makeAfterTrainingAdamState() -> FastGSAdamState {
    FastGSAdamState(
        ...
        opacities: makeAfterTrainingAdamField(shape: [3], start: 40),
